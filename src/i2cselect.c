...
typedef struct { int bus; char name[MAX_SHORTCUT_LENGTH];
		 access_type type; access_mode mode;
		 access_expect expect; int device_address;
		 int register_address; int expect_value;
		 char exec_before[MAX_SHELL_COMMAND_LENGHT];
		 char exec_after[MAX_SHELL_COMMAND_LENGHT]; } shortcut_struct;
...
//	HAS_EXEC_AFTER		= 0b1000000;  
	...
	switch (option) {
		...
		case HAS_EXEC_AFTER:
			...
			return (bitread>>6);
			break;
		default: return -1; // <-- should never be reached!!
	}
	return -1;
}
			...
			if (testkey[0] == 'b' || testkey[0] == 'B') {	// BUS
				...
				if (strcmp(to_lowercase(testkey), "expect=") == 0) {
					if (strcmp(to_lowercase(*(segments+i)), "expect=unset_expect") == 0) {
						...
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> EXPECTED OPERATION RESULT: Not set (default).\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "expect=value") == 0) {
						tempexpect = VALUE;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> EXPECTED OPERATION RESULT: Specified Value.\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "expect=none") == 0) {
						tempexpect = NONE;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> EXPECTED OPERATION RESULT: None or  not relevant (default).\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "expect=fail") == 0) {
						tempexpect = FAIL;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> EXPECTED OPERATION RESULT: Read-Error or errornous Reading.\n"); }
					} else {
						if (verbose) {
							fprintf(VERBOSE_OUT_STREAM, "Argument (\"%s\") is malformed, ignoring.\n", *(segments+i));
						}
				...
				if (strcmp(to_lowercase(testkey), "mode=") != 0) {
					...
					if (strcmp(to_lowercase(*(segments+i)), "mode=unset_mode") == 0) {
						tempmode = UNSET_MODE;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> ACCESS MODE: Not set (default).\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "mode=word") == 0) {
						tempmode = WORD;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> ACCESS MODE: Reading DWord.\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "mode=byte") == 0) {
						tempmode = BYTE;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> ACCESS MODE: Single Byte Read (default Mode).\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "mode=rwbyte") == 0) {
						tempmode = RWBYTE;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> ACCESS MODE: Read/Write Access to read Byte value.\n"); }
						...
						tempmode = WORD_PEC;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> ACCESS MODE: Reading DWord with Packet Error Checking (PEC).\n"); }
						...
						tempmode = BYTE_PEC;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> ACCESS MODE: Single Byte Read with Packet Error Checking (PEC).\n"); }
						...
						tempmode = RWBYTE_PEC;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> ACCESS MODE: Read/Write Access to read Byte value with Packet Error Checking (PEC).\n"); }
					} else {
						if (verbose) {
							fprintf(VERBOSE_OUT_STREAM, "Argument (\"%s\") is malformed, ignoring.\n", *(segments+i));
						}
				...
				if (strcmp(to_lowercase(testkey), "type=") != 0) {
					...
					if (strcmp(to_lowercase(*(segments+i)), "type=unset_type") == 0) {
						temptype = UNSET_TYPE;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> OPERATION TYPE: Not set (default).\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "type=bus") == 0) {
						temptype = BUS;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> OPERATION TYPE: Accessing Bus (default Type).\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "type=device") == 0) {
						temptype = DEVICE;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> OPERATION TYPE: Accessing Specified Device directly.\n"); }
					} else if (strcmp(to_lowercase(*(segments+i)), "type=register") == 0) {
						temptype = REGISTER;
						if (verbose) { fprintf(VERBOSE_OUT_STREAM, " -> OPERATION TYPE: Accessing Specified Register of certain Device.\n"); }
					} else {
						if (verbose) {
							fprintf(VERBOSE_OUT_STREAM, "Argument (\"%s\") is malformed, ignoring.\n", *(segments+i));
						}
